"""
Local fixtures for governance tests, mirroring evidence/tests/conftest.py
and remediation/tests/conftest.py 1:1 (two synthetic organisations, two
users, one membership each, two independent logged-in clients) so
governance tests do not depend on importing fixtures across an app
boundary from apps that are read-only to this dispatch.
"""
import pytest
...
@pytest.fixture
def make_user(db):
    def _make_user(username, password="a-strong-test-password-123", **extra):
        User = get_user_model()
