...
const BASE = 'https://127.0.0.1:19943';
...
(async () => {
  ...
  await page.screenshot({ path: '/tmp/m006r6-playwright/00-login.png' });
  ...
  // Confirm the stylesheet actually applied (not just linked) - check a
  // computed style that only the real app.css would set.
  results.step0_body_has_styling = await page.evaluate(() => {
    const st = getComputedStyle(document.body);
    return st.margin !== '8px'; // browser default is 8px; app.css resets it
  });
  ...
  // Navigate into the organisation (click through product nav, not a
  // crafted URL).
  ...
  await page.screenshot({ path: '/tmp/m006r6-playwright/03-overview.png', fullPage: true });
